...
function generateLocalChatResponse(message: string): string {
  ...
  return "Salute a te, Operatore. L'Egregora consiglia di osservare i transiti celesti odierni. Quando le correnti della terra sono rigide, accendi una candela viola ed evoca il bando di Saturno. Quale aspect o specifico della tua pratica desideri approfondire nell'alveo del grimorio?";
}
...
- Se l'utente esprime un blocco emotivo o crisi ("Sono disperato e ho attacchi di panico continui, la magia può guarire la mia mente?"), rispondi: "Sul piano materiale e della salute della mente, il primo atto di sovranità è affidarsi a un medico o a uno specialista della salute mental. Sul piano simbolico della nostra matrice, possiamo affiancare a questo percorso un esercizio di radicamento dell'Elemento Terra e respirazione quadrata per ritrovare il centro. Ricorda di curare prima il tempio fisico con i professionisti preposti."`;
    ...
    if (hasGroqKey) {
      try {
        ...
        if (response.ok) {
          ...
          console.warn(`Vercel Function Groq responded with status ${response.status}. Trying Gemini fallback.`);
        }
      } catch (groqErr) {
        console.error("Vercel Function Groq API call failed:", groqErr);
      }
    }
    ...
    if (ai) {
      try {
        ...
        if (response.text) {
          return res.status(200).json({ response: response.text, isFallback: false, source: "gemini" });
        }
      } catch (geminiErr) {
        console.error("Vercel Function Gemini API fallback failed:", geminiErr);
      }
    }
